...
// Removed timeout protection hook to simplify

interface AdminUsuariosFixedProps {
  onVoltar: () => void;
}
...
export function AdminUsuariosFixed({ onVoltar }: AdminUsuariosFixedProps) {
  ...
  if (!usuarioLogado) {
    console.log('[ADMIN_USUARIOS_FIXED] Usuário não logado, redirecionando...');
    return (
  ...
  // Verificação das variáveis do Supabase
  if (!projectId || !publicAnonKey) {
    console.error('[ADMIN_USUARIOS_FIXED] Variáveis do Supabase não encontradas:', { projectId, publicAnonKey });
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <p className="text-red-600 mb-4">Erro de configuração do Supabase</p>
          <Button onClick={onVoltar}>Voltar</Button>
        </div>
      </div>
    );
  ...
  // Removed complex timeout protection to avoid issues
  ...
  const carregarUsuarios = async () => {
    ...
    try {
      // Ultra-simplified request with aggressive timeout
      ...
      console.log('[ADMIN_USUARIOS_FIXED] Dados recebidos:', data);
      ...
      if (error.name === 'AbortError') {
        errorMessage = '⏱️ Timeout: Requisição demorou mais de 3 segundos.';
        toast.error('Timeout', {
          description: 'O servidor não respondeu a tempo.'
        });
        
        // Redirect to emergency mode after 2 timeouts
        const timeoutCount = parseInt(localStorage.getItem('timeout_count') || '0') + 1;
        localStorage.setItem('timeout_count', timeoutCount.toString());
  ...
  const salvarEdicao = async () => {
    ...
    try {
      ...
      console.error('[ADMIN_USUARIOS_FIXED] Erro ao salvar usuário:', error);
  ...
  const alterarStatusUsuario = async (usuario: Usuario, novoStatus: boolean) => {
    try {
      ...
      console.error('[ADMIN_USUARIOS_FIXED] Erro ao alterar status:', error);
  ...
  useEffect(() => {
    // Debounce the initial load to prevent multiple calls
