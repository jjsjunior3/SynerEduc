...
import { ArrowLeft, User, Mail, Calendar, Search, Loader2, Edit, Trash2, Shield, ShieldOff, Key, Save, X } from 'lucide-react';
...
export function GerenciarUsuarios({ onVoltar }: GerenciarUsuariosProps) {
  ...
  const carregarUsuarios = async () => {
    ...
    try {
      ...
      if (error.message.includes('Failed to fetch') || error.message.includes('NetworkError')) {
        ...
        toast.error(
          '🚨 ERRO CRÍTICO: Failed to fetch - Sistema corrompido',
          {
            duration: 30000,
            action: {
              label: '🆘 CORREÇÃO DE EMERGÊNCIA',
              onClick: () => window.open('?ultima-emergencia', '_blank')
            }
          }
        );
        
        // Log crítico
        console.error('[ADMIN_USUARIOS] ERRO CRÍTICO NO CARREGAMENTO:', {
        ...
        toast.error(
          'ERRO 404: Rotas não encontradas',
          {
            duration: 15000,
            action: {
              label: '🔧 Corrigir Rotas',
              onClick: () => window.open('?teste-rotas-emergencia', '_blank')
            }
          }
        );
  ...
  const salvarEdicao = async () => {
    ...
    try {
      ...
      // Detectar tipo de erro e mostrar correção específica
      if (error.message && error.message.includes('Usuário não encontrado')) {
        toast.error(
          `${error.message} - Correção disponível`,
          { 
            duration: 15000,
            action: {
              label: '🔧 Corrigir',
              onClick: () => window.open('?usuario-nao-encontrado', '_blank')
            }
          }
        );
        ...
        // Erro crítico do KV Store - REDIRECIONAR PARA CORREÇÃO DE EMERGÊNCIA
        toast.error(
        ...
        console.error('[ADMIN_USUARIOS] ERRO CRÍTICO DETECTADO:', {
          erro: error.message,
          timestamp: new Date().toISOString(),
          stack: error.stack
        });
  ...
  const alterarStatusUsuario = async (usuario: Usuario, novoStatus: boolean) => {
    try {
      const response = await fetch(`https://${projectId}.supabase.co/functions/v1/make-server-c61d1ad0/admin/usuarios/${usuario.id}/status`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${publicAnonKey}`,
          'Content-Type': 'application/json'
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Buscar, editar e gerenciar usuários do sistema</p>
            </div>
          </div>
