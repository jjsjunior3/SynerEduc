...
import { ArrowLeft, Plus, Edit2, Trash2, BookOpen, GraduationCap, AlertCircle, Monitor, Users } from "lucide-react";
...
interface Disciplina {
  ...
  segmento: "ead" | "presencial" | null; ativa: boolean;
}

interface Serie {
  id: string; nome: string; nivel: string;
  segmento: "ead" | "presencial" | null;
  totalAlunos: number; ativa: boolean;
}
...
function labelNivel(nivel: string) {
  const map: Record<string, string> = { fundamental: "Ens. Fundamental", medio: "Ens. Médio" };
...
function GestaoEscola({ onVoltar, segmentoForcado }: GestaoEscolaProps) {
  ...
  const [filtroDisciplina, setFiltroDisciplina] = useState<"todos" | "ead" | "presencial">(
    segmentoForcado ?? "todos"
  );
  const [filtroSerie, setFiltroSerie] = useState<"todos" | "ead" | "presencial">(
    segmentoForcado ?? "todos"
  );

  const [formDisciplina, setFormDisciplina] = useState({
    nome: "", descricao: "", segmento: (segmentoForcado ?? "") as "ead" | "presencial" | "", ativa: true,
  });

  const [formSerie, setFormSerie] = useState({
    nome: "", nivel: "fundamental" as string,
  ...
  async function carregarSeries() {
    try {
      ...
      const { data: contagemData } = await supabase
        .from("users").select("serie").eq("tipo", "aluno")
        .eq("status", "ativo").not("serie", "is", null);
  ...
  const resetForms = () => {
    setFormDisciplina({ nome: "", descricao: "", segmento: segmentoForcado ?? "", ativa: true });
    setFormSerie({ nome: "", nivel: "fundamental", segmento: segmentoForcado ?? "", ativa: true });
  };
  ...
  const handleSalvarDisciplina = async () => {
    ...
    const payload: any = {
      nome: formDisciplina.nome.trim(), descricao: formDisciplina.descricao.trim() || null,
    ...
    try {
      if (editando) {
        ...
        if (error) throw error;
        setDisciplinas(prev => prev.map(d => d.id === editando.id ? { ...d, ...payload } : d));
        toast.success("Disciplina atualizada!");
      } else {
        const { data, error } = await supabase.from("disciplinas").insert(payload).select("id").single();
        if (error) throw error;
        setDisciplinas(prev => [...prev, { id: data?.id || Date.now().toString(), ...payload }]);
  ...
  const handleSalvarSerie = async () => {
    ...
    try {
      if (editando) {
        ...
        if (error) throw error;
        setSeries(prev => prev.map(s => s.id === editando.id ? { ...s, ...payload } : s));
        toast.success("Série atualizada!");
      } else {
        const { data, error } = await supabase.from("series").insert(payload).select("id").single();
        if (error) throw error;
        setSeries(prev => [...prev, { id: data?.id || Date.now().toString(), totalAlunos: 0, ...payload }]);
  ...
  const disciplinasFiltradas = disciplinas.filter(d =>
    filtroDisciplina === "todos" ? true : d.segmento === filtroDisciplina
  );
  const seriesFiltradas = series.filter(s =>
    filtroSerie === "todos" ? true : s.segmento === filtroSerie
  );
  ...
  // Opções de segmento visíveis nos filtros
  const opcoesSegmento = (["todos", "ead", "presencial"] as const).filter(seg =>
    !segmentoForcado || seg === segmentoForcado
  );
          ...
          <Tabs value={abaSelecionada} onValueChange={(v) => setAbaSelecionada(v as any)}>
              ...
              <div className="flex flex-wrap justify-between items-center gap-3">
                <div className="flex items-center gap-1 rounded-lg border border-border bg-muted p-1">
                  {opcoesSegmento.map(seg => (
                    <button key={seg} onClick={() => setFiltroDisciplina(seg)}
                      ...
                      <span className="ml-1.5 text-muted-foreground">({contDisciplinas[seg]})</span>
                    </button>
                  ))}
                </div>
                  ...
                  <div key={disciplina.id}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {disciplina.descricao && <p className="text-sm text-muted-foreground">{disciplina.descricao}</p>}
                        <div className="flex gap-1.5 mt-1 flex-wrap">
                          {disciplina.segmento && (
                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${badgeSegmento(disciplina.segmento)}`}>
                              {disciplina.segmento === "ead" ? <Monitor className="w-3 h-3 mr-1" /> : <Users className="w-3 h-3 mr-1" />}
                    ...
                    <div className="flex items-center gap-1">
                      <Button variant="ghost" size="sm" onClick={() => handleAbrirModal("disciplina", disciplina)}>
                        <Edit2 className="w-4 h-4" />
                      </Button>
                      <Button variant="ghost" size="sm" onClick={() => handleExcluirDisciplina(disciplina.id)}
            ...
            <TabsContent value="series" className="space-y-4">
              <div className="flex flex-wrap justify-between items-center gap-3">
                <div className="flex items-center gap-1 rounded-lg border border-border bg-muted p-1">
                  {opcoesSegmento.map(seg => (
                    <button key={seg} onClick={() => setFiltroSerie(seg)}
                      className={`px-3 py-1 rounded-md text-xs font-medium transition-colors ${
                        filtroSerie === seg
                          ? "bg-background text-foreground shadow-sm"
                          : "text-muted-foreground hover:text-foreground"
                      ...
                      {seg === "todos" ? "Todas" : seg === "ead" ? "EAD" : "Presencial"}
                      <span className="ml-1.5 text-muted-foreground">({contSeries[seg]})</span>
                    </button>
                  ))}
                </div>
              ...
              <div className="space-y-2">
                {seriesFiltradas.length === 0 ? (
                  ...
                  <div key={serie.id}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex gap-1.5 mt-1 flex-wrap">
                          ...
                          {serie.segmento && (
                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${badgeSegmento(serie.segmento)}`}>
                              {serie.segmento === "ead" ? <Monitor className="w-3 h-3 mr-1" /> : <Users className="w-3 h-3 mr-1" />}
                    ...
                    <div className="flex items-center gap-1">
                      <Button variant="ghost" size="sm" onClick={() => handleAbrirModal("serie", serie)}>
                        <Edit2 className="w-4 h-4" />
                      </Button>
                      <Button variant="ghost" size="sm" onClick={() => handleExcluirSerie(serie.id)}
                        className="text-destructive hover:text-destructive">
                        <Trash2 className="w-4 h-4" />
                      </Button>
                    </div>
            ...
            {abaSelecionada === "disciplinas" ? (
              <>
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Input id="nomeDisciplina" value={formDisciplina.nome}
                    onChange={e => setFormDisciplina({ ...formDisciplina, nome: e.target.value })}
                    placeholder="Ex: Matemática" className="col-span-3" />
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="descricaoDisciplina" className="text-right text-foreground">Descrição</Label>
                  <Input id="descricaoDisciplina" value={formDisciplina.descricao}
                    onChange={e => setFormDisciplina({ ...formDisciplina, descricao: e.target.value })}
                    placeholder="Opcional" className="col-span-3" />
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="segmentoDisciplina" className="text-right text-foreground">Segmento *</Label>
                  <select id="segmentoDisciplina" value={formDisciplina.segmento}
                    disabled={!!segmentoForcado}
                    onChange={e => setFormDisciplina({ ...formDisciplina, segmento: e.target.value as any })}
                ...
                <div className="flex items-center gap-2 justify-end col-span-4">
                  <Checkbox id="ativaDisciplina" checked={formDisciplina.ativa}
                    onCheckedChange={checked => setFormDisciplina({ ...formDisciplina, ativa: !!checked })} />
                  <label htmlFor="ativaDisciplina" className="text-sm font-medium text-foreground cursor-pointer">
                    Disciplina ativa
                  </label>
                </div>
              </>
              ...
              <>
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Input id="nomeSerie" value={formSerie.nome}
                    onChange={e => setFormSerie({ ...formSerie, nome: e.target.value })}
                    placeholder="Ex: 1º Ano" className="col-span-3" />
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="nivelSerie" className="text-right text-foreground">Nível *</Label>
                  <select id="nivelSerie" value={formSerie.nivel}
                    onChange={e => setFormSerie({ ...formSerie, nivel: e.target.value })}
                    className={selectClass}>
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <select id="segmentoSerie" value={formSerie.segmento}
                    disabled={!!segmentoForcado}
                    onChange={e => setFormSerie({ ...formSerie, segmento: e.target.value as any })}
                    className={selectClass}>
                    <option value="">Selecione o segmento</option>
                    <option value="ead">EAD</option>
                    <option value="presencial">Presencial</option>
                ...
                <div className="flex items-center gap-2 justify-end col-span-4">
                  <Checkbox id="ativaSerie" checked={formSerie.ativa}
                    onCheckedChange={checked => setFormSerie({ ...formSerie, ativa: !!checked })} />
                  <label htmlFor="ativaSerie" className="text-sm font-medium text-foreground cursor-pointer">
                    Série ativa
                  </label>
                </div>
              </>
