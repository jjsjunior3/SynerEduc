...
interface ComunicadoForm {
  ...
  usuarioUnico: string | null; usuarioUnicoNome: string;
  prioridade: 'baixa' | 'media' | 'alta';
  agendarEnvio: boolean; dataEnvio?: string; horaEnvio?: string;
  arquivo: File | null;
}
...
export default function EnviarComunicado({ onVoltar }: EnviarComunicadoProps) {
  ...
  // ── Carregar grupos de destino filtrados pelo segmento ──
  useEffect(() => {
    async function carregarGrupos() {
      try {
        ...
        // Séries filtradas pelo segmento do coordenador
  ...
  // ── Busca individual com debounce (filtra pelo segmento) ──
  useEffect(() => {
  ...
  }, [buscaUsuario, modoDestinatario, segmento]);

  // ── Histórico: filtra pelo segmento do coordenador ──
  useEffect(() => {
    if (aba === 'historico') carregarHistorico();
  }, [aba, filtroHistoricoInicio, filtroHistoricoFim, segmento]);
  ...
  async function carregarHistorico() {
    ...
    try {
      const { data, error } = await supabase
        ...
        .in('segmento', [segmento, 'todos'])   // ← filtro: segmento próprio + 'todos'
  ...
  function handleArquivoChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file.type.startsWith('image/')) setArquivoPreview(URL.createObjectURL(file));
  ...
  async function uploadArquivo(file: File): Promise<string | null> {
    try {
      ...
      const nomeSeguro = file.name.replace(/[^a-zA-Z0-9._-]/g, '_').replace(/_+/g, '_');
      const path = `${timestamp}_${nomeSeguro}`;
      const { error } = await supabase.storage.from('comunicados').upload(path, file, { cacheControl: '3600', upsert: false });
    ...
    } catch { toast.error('Erro inesperado no upload.'); return null; }
  }

  // ── Enviar comunicado — salva segmento ──
  async function handleEnviar() {
    if (!usuario?.id) { toast.error('Você precisa estar logado.'); return; }
    if (!form.titulo.trim() || !form.conteudo.trim()) { toast.error('Preencha o título e o conteúdo.'); return; }
    ...
    if (!destinatarioFinal) {
      toast.error(modoDestinatario === 'individual' ? 'Selecione um usuário.' : 'Selecione ao menos um destinatário.');
      return;
    }
    ...
    try {
      ...
      if (form.arquivo) {
        ...
        if (!arquivoUrl) {
          toast.error('Não foi possível fazer o upload do arquivo. Tente novamente ou remova o anexo antes de enviar.');
          return;
        }
      }
      ...
      const { error } = await supabase.from('comunicados').insert({
        ...
        segmento: segmento,                    // ← salva segmento do coordenador
      });
      if (error) throw error;
      ...
      setArquivoPreview(null);
      setBuscaUsuario(''); setResultadosBusca([]);
      if (inputArquivoRef.current) inputArquivoRef.current.value = '';
    } catch (err: any) {
  ...
  function abrirEdicao(item: ComunicadoHistorico) {
    ...
    setFormEdicao({ titulo: item.titulo, conteudo: item.conteudo, importante: item.importante });
  }

  async function salvarEdicao() {
    if (!editandoId) return;
    if (!formEdicao.titulo.trim() || !formEdicao.conteudo.trim()) { toast.error('Preencha o título e o conteúdo.'); return; }
    ...
    try {
      ...
      setHistorico(prev => prev.map(c =>
        c.id === editandoId ? { ...c, ...formEdicao } : c
      ));
      setEditandoId(null);
  ...
  const confirmarDeletar = async () => {
    ...
    try {
      ...
      setHistorico(prev => prev.filter(c => c.id !== confirmarDelete));
  ...
  return (
    <>
            ...
            return (
              <button key={tab.id} onClick={() => setAba(tab.id as any)}
                className={`flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 transition-colors ${
                  aba === tab.id ? 'border-blue-600 text-blue-600' : 'border-transparent text-muted-foreground hover:text-foreground'
                }`}>
            ...
            <CardHeader className="border-b border-border pb-4">
              <div className="flex items-start justify-between">
                ...
                <span className="text-xs px-2.5 py-1 rounded-full font-medium border
                  ...
                  dark:bg-emerald-900/40 dark:text-emerald-300 dark:border-emerald-700 capitalize">
                  {segmento}
                </span>
              </div>
            </CardHeader>
            <CardContent className="pt-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="space-y-2">
                  ...
                  <Input value={form.titulo}
                    ...
                    placeholder="Digite o título do comunicado" disabled={loadingEnvio} />
                </div>
                <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <Textarea value={form.conteudo}
                  ...
                  placeholder="Digite o conteúdo do comunicado..." rows={6} disabled={loadingEnvio} />
              </div>
              ...
              <div className="space-y-2">
                <Label className="text-foreground font-medium flex items-center gap-2">
                  ...
                  Anexo <span className="text-muted-foreground text-xs font-normal">(opcional — máx. 10MB)</span>
                </Label>
                {form.arquivo ? (
                    ...
                    {arquivoPreview ? (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">{(form.arquivo.size / 1024 / 1024).toFixed(2)} MB</p>
                        </div>
                      </div>
                    )}
                  </div>
                ) : (
                  <div onClick={() => inputArquivoRef.current?.click()}
                ...
                <input ref={inputArquivoRef} type="file"
                  ...
                  className="hidden" onChange={handleArquivoChange} />
              </div>
              ...
              <div className="space-y-4 pt-2 border-t border-border">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-1 p-0.5 bg-muted rounded-lg">
                    {(['grupo', 'individual'] as const).map(modo => (
                      <button key={modo} onClick={() => setModoDestinatario(modo)}
                        className={`px-3 py-1.5 text-xs rounded-md font-medium transition-colors ${
                          modoDestinatario === modo ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'
                        }`}>
                        {modo === 'grupo' ? 'Grupos' : 'Usuário Único'}
                      </button>
                    ))}
                  </div>
                </div>
                      ...
                      <div key={g.id} className="flex items-center gap-2">
                        <Checkbox id={g.id}
                          ...
                          onCheckedChange={c => setForm(p => ({
                            ...p,
                            destinatarios: c ? [...p.destinatarios, g.id] : p.destinatarios.filter(d => d !== g.id),
                          }))}
                          disabled={loadingEnvio} />
                  ...
                  <div className="space-y-3">
                    {form.usuarioUnico ? (
                      <div className="flex items-center justify-between p-3 rounded-lg border border-border bg-muted/20">
                        ...
                        <button onClick={() => setForm(p => ({ ...p, usuarioUnico: null, usuarioUnicoNome: '' }))}
                      ...
                      <div className="relative">
                        <div className="relative">
                          ...
                          <Input className="pl-9" placeholder="Buscar aluno ou professor pelo nome..."
                            value={buscaUsuario} onChange={e => setBuscaUsuario(e.target.value)} disabled={loadingEnvio} />
                          {buscando && <Loader2 className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 animate-spin text-muted-foreground" />}
                        </div>
                        {resultadosBusca.length > 0 && (
                          <div className="absolute z-10 top-full mt-1 w-full rounded-lg border border-border shadow-xl overflow-hidden bg-card">
                            {resultadosBusca.map(u => (
                              <button key={u.id}
                                onClick={() => { setForm(p => ({ ...p, usuarioUnico: u.id, usuarioUnicoNome: u.nome })); setBuscaUsuario(''); setResultadosBusca([]); }}
                                ...
                                <div>
                                  ...
                                  <p className="text-xs text-muted-foreground capitalize">{u.tipo}{u.serie ? ` • ${u.serie}` : ''}</p>
                                </div>
                              </button>
                            ))}
                          </div>
              ...
              <div className="space-y-4 pt-4 border-t border-border">
                <div className="flex items-center gap-2">
                  <Checkbox id="agendarEnvio" checked={form.agendarEnvio}
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-border">
                <Button variant="outline" onClick={() => {
                  setForm({ titulo: '', conteudo: '', destinatarios: [], usuarioUnico: null, usuarioUnicoNome: '', prioridade: 'media', agendarEnvio: false, arquivo: null });
                  setArquivoPreview(null);
                }} disabled={loadingEnvio}>Limpar</Button>
                <Button className="gap-2" onClick={handleEnviar} disabled={loadingEnvio}>
                  {loadingEnvio
              ...
              <CardContent className="pt-5">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5 items-end">
                  ...
                  <Button variant="outline"
                    onClick={() => { setFiltroHistoricoInicio(mesAtual.inicio); setFiltroHistoricoFim(mesAtual.fim); }}>
                    Mês Atual
                  </Button>
                </div>
              </CardContent>
              ...
              <CardContent className="pt-5">
                {loadingHistorico ? (
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-muted-foreground text-xs">Ajuste as datas do filtro.</p>
                  </div>
                ) : (
                      ...
                      <div key={item.id} className="rounded-lg border border-border bg-card p-5 hover:shadow-md transition-shadow">
                        ...
                        <div className="flex items-center gap-2 pt-3 border-t border-border">
                          ...
                          <Button variant="outline" size="sm"
                            ...
                            onClick={() => pedirConfirmacaoDelete(item.id)}>
          ...
          <DialogContent className="max-w-lg">
            ...
            <div className="space-y-5 py-4">
              ...
              <div className="space-y-2">
                ...
                <Textarea value={formEdicao.conteudo}
                  onChange={e => setFormEdicao(p => ({ ...p, conteudo: e.target.value }))}
                  rows={6} disabled={salvandoEdicao} />
              </div>
              <div className="flex items-center gap-2">
                <Checkbox id="editImportante" checked={formEdicao.importante}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Excluir este comunicado?</AlertDialogTitle>
          </AlertDialogHeader>
