...
import {
  Printer, TrendingUp, Loader2, AlertTriangle, Info,
  ChevronDown, ChevronUp,
} from 'lucide-react';

interface UsuarioProps { id: string; nome: string; serie?: string; }
interface DisciplinaData { id: string; nome: string; cor?: string; }
interface TurmaProps {
  id: string; nome: string; serieId: string; serieNome: string;
  totalAlunos: number; disciplinas: DisciplinaData[];
}
interface BoletimProps { usuario: UsuarioProps | null; turma: TurmaProps | null; }

interface NotaBimestre {
  av1: number | null; av2: number | null; av3: number | null;
  rec: number | null; med: number | null;
}
...
interface NotaDisciplina {
  ...
  mediaFinal: number | null;   // só preenchido quando os 4 bimestres têm média
  recupFinal: number | null;
  situacao: 'Aprovado' | 'Reprovado' | 'Recuperação' | 'Cursando' | 'N/A';
}
...
export default function Boletim({ usuario, turma }: BoletimProps) {
  ...
  const buscarBoletim = useCallback(async () => {
    ...
    try {
      ...
      // Inicializa estrutura para todas as disciplinas da turma
      ...
      turma.disciplinas.forEach(disc => {
        notasPorDisciplina[disc.id] = {
          ...
          ptsTotal: null, mediaFinal: null, recupFinal: null,
          situacao: 'Cursando',
        };
      });

      // Preenche com as notas existentes
      (notasExistentes || []).forEach((nota: any) => {
        ...
        // Recalcula a média via utilitário — nunca usa o valor do banco
        ...
        const notaBimestre: NotaBimestre = {
          ...
          med: resultado.mediaFinal,  // média já com REC aplicada
        };
      ...
      // ── Calcula ptsTotal, mediaFinal e situação ─────────────────────────
      Object.values(notasPorDisciplina).forEach(nd => {
        ...
        const mediasComValor = medias.filter(m => m !== null) as number[];

        // Pts Total = soma das médias disponíveis (para exibição parcial)
        nd.ptsTotal = mediasComValor.length > 0
          ? mediasComValor.reduce((s, m) => s + m, 0)
          : null;

        // ── BUG 1 CORRIGIDO: mediaFinal só existe quando os 4 bimestres têm média ──
        if (mediasComValor.length === 4) {
          nd.mediaFinal = nd.ptsTotal! / 4;
        } else {
          nd.mediaFinal = null; // exibe '-' enquanto não tiver os 4 bimestres
        }

        // ── BUG 2 CORRIGIDO: situação 'Cursando' enquanto não tiver os 4 bimestres ──
        if (nd.mediaFinal === null) {
          // Ainda não concluiu os 4 bimestres — sempre Cursando
          nd.situacao = 'Cursando';
        } else {
          // Todos os 4 bimestres preenchidos — determina situação final
      ...
      setNotasBoletim(
        Object.values(notasPorDisciplina)
  ...
  const getSituacaoColor = (situacao: NotaDisciplina['situacao']) => {
    switch (situacao) {
      case 'Aprovado':    return 'bg-green-500/10 text-green-600 dark:text-green-400 border-green-500/20';
      case 'Recuperação': return 'bg-yellow-500/10 text-yellow-600 dark:text-yellow-400 border-yellow-500/20';
      case 'Reprovado':   return 'bg-red-500/10 text-red-600 dark:text-red-400 border-red-500/20';
      case 'Cursando':    return 'bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20';
  ...
  const getSituacaoBorderColor = (s: NotaDisciplina['situacao']) => {
    switch (s) {
      ...
      case 'Cursando':    return 'border-l-blue-400';
  ...
  const colsHeader = colunasBimestre.map((col, i) => (
    <th key={i} className="border border-border px-2 py-1 text-center">{col}</th>
  ));
  ...
  const handleImprimir = () => {
    ...
    const linhasTabela = notasBoletim.map(nota => `
      <tr>
        ...
        ${linhasBimestre(nota.bimestre1)}
        ${linhasBimestre(nota.bimestre2)}
        ${linhasBimestre(nota.bimestre3)}
        ${linhasBimestre(nota.bimestre4)}
        <td style="border:1px solid #ccc;padding:4px 6px;text-align:center;font-weight:700;color:#1d4ed8">
          ${nota.ptsTotal !== null ? nota.ptsTotal.toFixed(1) : '-'}
        </td>
        <td style="border:1px solid #ccc;padding:4px 6px;text-align:center;font-weight:700;font-size:14px;color:${nota.mediaFinal !== null ? (nota.mediaFinal >= 7 ? '#15803d' : nota.mediaFinal >= 5 ? '#ca8a04' : '#dc2626') : '#999'}">
          ${nota.mediaFinal !== null ? nota.mediaFinal.toFixed(1) : '-'}
        </td>
        <td style="border:1px solid #ccc;padding:4px 6px;text-align:center;color:#999">-</td>
        <td style="border:1px solid #ccc;padding:4px 6px;text-align:center;font-weight:600;color:${corSituacao(nota.situacao)}">
          ${nota.situacao}
        </td>
      </tr>`).join('');
...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .bim-1{background:#dbeafe}.bim-2{background:#dcfce7}.bim-3{background:#fef9c3}.bim-4{background:#ede9fe}
    tr:nth-child(even) td{background:#f9fafb}
    .legenda{margin-top:12px;display:flex;gap:40px;font-size:10px;color:#444;border-top:1px solid #ddd;padding-top:10px}
    .legenda ul{list-style:none}.legenda li{margin-bottom:3px}
...
<body>
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64 text-foreground">
        <Loader2 className="animate-spin w-6 h-6 mr-2" /> Carregando boletim...
      </div>
    );
  }
  ...
  // ── Card mobile por bimestre ──────────────────────────────────────────────
  const BimestreRow = ({
    label, bimestre, color,
  }: { label: string; bimestre: NotaBimestre; color: string }) => {
    const cols = isPresencial ? 6 : 5;
    return (
    ...
    <div className="space-y-6 print:p-0 print:m-0 print:space-y-0">
      ...
      <div className="flex justify-end print:hidden">
        <Button onClick={handleImprimir} className="bg-primary text-primary-foreground hover:bg-primary/90">
          <Printer className="w-4 h-4 mr-2" /> Imprimir Boletim
        </Button>
      </div>
      ...
      <div className="md:hidden space-y-3">
        ...
        {notasBoletim.map(nota => {
          ...
          return (
            <Card key={nota.disciplina_id}
              className={`border-l-4 ${getSituacaoBorderColor(nota.situacao)} overflow-hidden`}>
              <button
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    {nota.mediaFinal !== null ? (
                      <span className={`text-lg font-bold ${getNotaColor(nota.mediaFinal)}`}>
                        {nota.mediaFinal.toFixed(1)}
                      </span>
                    ) : (
                      <span className="text-sm text-muted-foreground">Em andamento</span>
                    )}
                    <Badge variant="outline" className={`text-[10px] ${getSituacaoColor(nota.situacao)}`}>
                      {nota.situacao}
                    </Badge>
                  </div>
                </div>
              ...
              {isOpen && (
                <div className="px-4 pb-4 space-y-2">
                  <div className="grid gap-1 text-[10px] text-muted-foreground px-3"
                  ...
                  <div className="flex items-center gap-4 pt-2 border-t border-border mt-2 text-xs">
                    <span className="text-muted-foreground">
                      Pts Total: <strong className="text-primary">
                        {nota.ptsTotal !== null ? nota.ptsTotal.toFixed(1) : '-'}
                      </strong>
                    </span>
                    <span className="text-muted-foreground">
                      Média Final: <strong className={getNotaColor(nota.mediaFinal)}>
                        {nota.mediaFinal !== null ? nota.mediaFinal.toFixed(1) : '-'}
                      </strong>
                    </span>
                  </div>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="bg-muted/50 print:!bg-gray-200">
                  <th rowSpan={2} className="border border-border px-3 py-2 text-left font-semibold text-foreground w-[18%]">
                    Disciplina
                  </th>
                  <th colSpan={colunasBimestre.length} className="border border-border px-2 py-1 text-center font-semibold bg-blue-500/10">1º Bimestre</th>
                  <th colSpan={colunasBimestre.length} className="border border-border px-2 py-1 text-center font-semibold bg-green-500/10">2º Bimestre</th>
                  <th colSpan={colunasBimestre.length} className="border border-border px-2 py-1 text-center font-semibold bg-yellow-500/10">3º Bimestre</th>
                  <th colSpan={colunasBimestre.length} className="border border-border px-2 py-1 text-center font-semibold bg-purple-500/10">4º Bimestre</th>
                  <th rowSpan={2} className="border border-border px-2 py-2 text-center font-semibold text-foreground">Pts Total</th>
                  <th rowSpan={2} className="border border-border px-2 py-2 text-center font-semibold text-foreground">Média Final</th>
                  <th rowSpan={2} className="border border-border px-2 py-2 text-center font-semibold text-foreground">Recup Final</th>
                  <th rowSpan={2} className="border border-border px-2 py-2 text-center font-semibold text-foreground">Situação</th>
                </tr>
                <tr className="bg-muted/30 text-xs text-muted-foreground">
                  {[0, 1, 2, 3].flatMap(b =>
                    colunasBimestre.map((col, i) => (
                      <th key={`b${b}-${i}`} className="border border-border px-2 py-1 text-center">{col}</th>
                    ))
                  )}
                </tr>
              </thead>
              <tbody>
                {notasBoletim.map((nota, idx) => {
                  const renderCelulaBim = (b: NotaBimestre) => (
                    <>
                      <td className={`border border-border px-2 py-2 text-center ${getNotaColor(b.av1)}`}>{fmtNota(b.av1)}</td>
                      <td className={`border border-border px-2 py-2 text-center ${getNotaColor(b.av2)}`}>{fmtNota(b.av2)}</td>
                      {isPresencial && (
                        <td className={`border border-border px-2 py-2 text-center ${getNotaColor(b.av3)}`}>{fmtNota(b.av3)}</td>
                      )}
                      <td className="border border-border px-2 py-2 text-center text-muted-foreground">
                        {b.rec !== null && b.rec > 0 ? b.rec.toFixed(1) : '-'}
                      </td>
                      <td className={`border border-border px-2 py-2 text-center font-bold ${getNotaColor(b.med)}`}>
                        {fmtNota(b.med)}
                      </td>
                    </>
                  );
                  ...
                  return (
                    <tr key={nota.disciplina_id || idx}
                      className="hover:bg-muted/30 transition-colors group">
                      <td className="border border-border px-3 py-2 font-semibold sticky left-0 bg-card group-hover:bg-muted/30 text-foreground z-10">
                      ...
                      <td className={`border border-border px-2 py-2 text-center font-bold text-lg ${getNotaColor(nota.mediaFinal)}`}>
                        {nota.mediaFinal !== null ? nota.mediaFinal.toFixed(1) : '-'}
                      </td>
                      <td className="border border-border px-2 py-2 text-center text-muted-foreground">
                      ...
                      <td className="border border-border px-2 py-2 text-center">
                        <Badge variant="outline" className={getSituacaoColor(nota.situacao)}>
                          {nota.situacao}
                        </Badge>
                      </td>
                    </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
            <div>
              ...
              <ul className="space-y-2 text-muted-foreground">
                <li><strong className="text-foreground">AV1/AV2{isPresencial ? '/AV3' : ''}:</strong> Avaliações</li>
                <li><strong className="text-foreground">REC:</strong> Recuperação do Bimestre</li>
                <li>
                  <strong className="text-foreground">MÉD:</strong>{' '}
                  {isPresencial ? 'Média do Bimestre (AV1 + AV2 + AV3) ÷ 3' : 'Média do Bimestre (AV1 + AV2) ÷ 2'}
                </li>
                <li><strong className="text-foreground">Pts Total:</strong> Soma das médias dos 4 bimestres</li>
                <li><strong className="text-foreground">Média Final:</strong> Pts Total ÷ 4 (somente após os 4 bimestres)</li>
              </ul>
            </div>
            <div>
              <h4 className="font-semibold text-foreground mb-3">Critérios de Aprovação:</h4>
              <ul className="space-y-3 text-muted-foreground">
                ...
                <li className="flex items-center gap-2">
                  ...
                  <span>Média Final entre 5.0 e 6.9: Recuperação</span>
                </li>
                <li className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-red-500" />
                  <span>Média Final &lt; 5.0: Reprovado</span>
                </li>
                <li className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-blue-400" />
                  <span>Sem os 4 bimestres: Cursando</span>
                </li>
              </ul>
            </div>
          </div>
